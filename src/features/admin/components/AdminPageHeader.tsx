...
const AdminPageHeader = ({ children, className }: { children: ReactNode, className?: string }) => {
    return (
        <div className={cn("flex justify-between items-end p-4", className)}>{children}</div>
    )
}
...
function Meta({ children, className }: { children: ReactNode, className?: string }) {
    return (
        <p className={cn("uppercase text-[10px] text-primary font-medium tracking-[0.22em]", className)}>{children}</p>
    )
}
